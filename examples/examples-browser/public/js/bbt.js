const classes = ['kenneth', 'cleo', 'joel', 'rash', 'michael_s', 'rosaldo', 'manz', 'stephan', 'sam', 'rj', 'michael_a', 'george', 'jason', 'howard', 'ryan', 'uffe', 'richard', 'michael_f', 'pam']
...
async function createBbtFaceMatcher(numImagesForTraining = 2) {
  ...
  const labeledFaceDescriptors = await Promise.all(classes.map(
    async className => {
      const descriptors = []
      for (let i = 2; i < (numImagesForTraining + 1); i++) {
